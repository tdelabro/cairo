...
use cairo_lang_vm_utils::{execute_core_hint_base, extract_buffer, get_ptr, get_val};
...
pub fn run_function<'a, 'b: 'a, Instructions: Iterator<Item = &'a Instruction> + Clone>(
    ...
    // Safe because no identifiers has been passed
    .unwrap();
    ...
    runner
        ...
        .map_err(VirtualMachineError::from)
        .map_err(Box::new)?;
    runner.relocate(&mut vm, true).map_err(VirtualMachineError::from).map(Box::new)?;
    Ok((
